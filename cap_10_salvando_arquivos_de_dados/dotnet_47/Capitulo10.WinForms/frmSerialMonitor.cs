...
namespace Capitulo10.WinForms
{
    ...
    {        
        ...
        List<string> respostaList = new List<string>();
        ...
        private void trataDadoRecebido(string content)
        {
            if (content.Length >= 8)
            {
                ...
                {
                    var conteudo = content.Substring(4, 4);
                    var dataHora = DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss");
                    var itemResposta = $"{dataHora}    {conteudo}";
                    respostaList.Add(itemResposta);
                    lblControle.Text = conteudo;
        ...
        {
            if (sfvLog.ShowDialog() == DialogResult.OK)
            {
                var filePath = sfvLog.FileName;

                using (var file = File.AppendText(sfvLog.FileName))
                {
                    respostaList.ForEach(resposta => file.WriteLine(resposta));
                    file.Close();
                }

                lblStatusArquivo.Text = $"Arquivo {filePath} salvo com sucesso.";
            }
        }
    }
}
